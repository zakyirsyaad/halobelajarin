...
export default function Testimoni() {
    ...
    return (
        <div className='bg-secondary text-primary px-8 md:px-16 lg:px-24 xl:px-36 py-10'>
            <p className='text-third font-space font-bold text-4xl mb-10'>What they say about Belajarin</p>
            <div className='flex flex-col md:flex-row flex-wrap justify-between space-y-5 md:space-y-5'>
                {[testi1, testi2, testi3].map((testimonial, index) => (
                    <motion.div
                        key={index}
                        ref={ref}
                        style={{
                        ...
                        className='bg-[#EDEE94] w-full md:w-80 pb-5 rounded-xl text-secondary'>
                        <Image src={testimonial} alt={`Testimoni Belajarin ${index + 1}`} width={200} height={200} className='w-full rounded-br-[50px] border-4 border-secondary' />
                        <div className='flex flex-row items-center justify-between'>
                            <div className='flex flex-col p-3 w-full md:w-64 justify-between'>
                                <p className='font-bold mb-5'>Testimonial {index + 1}</p>
