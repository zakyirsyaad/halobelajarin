...
export default async function DigitalMarketing() {
    const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/digital-marketing`)
    const data = await response.json()

    return (
        <div className='flex flex-row flex-wrap justify-around'>
            {data.subcategories.map((subCategory, index) => (
                <div key={index} className='bg-blue-700 rounded-xl shadow-sm text-white p-5 mb-10 w-72 h-80 flex flex-col justify-between duration-200 hover:scale-105'>
                    ...
                    <div>
                        {subCategory.subMenu.map((menuItem, subIndex) => (
                            <div key={subIndex}>
                                <Link href={`/DigitalMarketing/${menuItem.uid}`}>
