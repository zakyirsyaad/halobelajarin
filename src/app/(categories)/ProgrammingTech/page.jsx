...
export default async function ProgrammingTech() {
    const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/programming`)
    const programming = await response.json()

    return (
        <div className='flex flex-row flex-wrap justify-around'>
            {programming.subcategories.map((subCategory, index) => (
                <div key={index} className='bg-blue-700 rounded-xl shadow-sm text-white p-5 mb-10 w-72 h-72 flex flex-col justify-between duration-200 hover:scale-105'>
                    ...
                    <div>
                        {subCategory.subMenu.map((menuItem, subIndex) => (
                            <div key={subIndex}>
                                <Link href={`/ProgrammingTech/${menuItem.uid}`}>
